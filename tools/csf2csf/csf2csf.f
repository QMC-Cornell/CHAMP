...
c Write out sum of squares of csf_coef.  If cutoff_g2q=0 in gamess2qmc then this should be 1, otherwise < 1.
      csf_sum=0
      do icsf=1,ncsf
        csf_sum=csf_sum+csf_coef(icsf)**2
      enddo
      csf_sum=sqrt(csf_sum)
      write(6,'(''For csfs selected by gamess2qmc and read in here, csf_sum='',f8.6)') csf_sum
...
c Write the line for making 0Info
      ...
      write(6,'(8i2)',advance='no') (icsf,icsf=2,min(ncsf,9))
      write(6,'(90i3)',advance='no') (icsf,icsf=10,min(ncsf,99))
      write(6,'(900i4)',advance='no') (icsf,icsf=100,min(ncsf,999))
      write(6,'(9000i5)',advance='no') (icsf,icsf=1000,min(ncsf,9999))
...
c     write(fmt,'(''(''i4,''f12.8,\'\' (csf_coef(icsf),icsf=1,ncsf)\'\')'')') ncsf
     ...
     &''Increase cutoff_g2q or decrease eps.'')') max_ndet_in_csf
...
c    &    write(6,'(''Warning:cdet_in_csf(idet_in_csf,icsf)='',f9.6)') cdet_in_csf(idet_in_csf,icsf)
  ...
  720   write(6,fmt) (nint(cdet_in_csf(idet_in_csf,icsf)),idet_in_csf=1,ndet_in_csf(icsf)),
     &  ' (cdet_in_csf(idet_in_csf,icsf),idet_in_csf=1,ndet_in_csf(icsf))'
      ...
      write(6,'(''Reduce nparm and nparm_csf by'',i6)') ncsf-ncsf_old

      if(max_abs_cdet_in_csf.gt.48) write(6,'(''Warning: max_abs_cdet_in_csf='',i5,'' Probably too many CSFs were combined. ''
     &''Increase cutoff_g2q or decrease eps.'')') max_abs_cdet_in_csf
