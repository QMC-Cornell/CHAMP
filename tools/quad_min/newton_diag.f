...
c e-e-n terms, but for the moment just check the e-n and e-e terms.
     ...
     &hess_sav(MDIM,MDIM),grad_sav(MDIM),grad_cal(MDIM)
      ...
      parameter(MBUF=1024)
      dimension work(MBUF+MDIM*MDIM),eigv(MDIM),hessi(MDIM,MDIM)
    ...
    8   hess(i,j)=hess_sav(i,j)
      ...
      write(6,'(''eigs='',(20d8.2)') (eigv(i),i=1,naprmj)

c Check that spectral decomposition is done correctly (recover hessian)
      do 11 i=1,nparmj
        do 11 j=1,nparmj
          hessi(i,j)=0
          do 11 k=1,nparmj
   11       hessi(i,j)=hessi(i,j)+hess(i,k)*eigv(k)*hess(j,k)

      do 12 i=1,nparmj
   12   write(6,'(''hess_cal='',9g12.4)') (hessi(i,j),j=1,i)

      do 15 i=1,nparmj
c       if(eigv(i).gt.1.d-2) then
        if(eigv(i).gt.0.d0) then
c         eigv(i)=1/eigv(i)
          eigv(i)=1/(eigv(i)+add_diag)
         else
          eigv(i)=0
        endif
        write(6,*) 'inverse eigenvalue ',eigv(i)
   15 continue
      ...
      do 20 i=1,nparmj
        do 20 j=1,nparmj
          hessi(i,j)=0
          do 20 k=1,nparmj
   20       hessi(i,j)=hessi(i,j)+hess(i,k)*eigv(k)*hess(j,k)
 
c     do 25 i=1,nparmj
c       do 23 j=1,nparmj
c         work(j)=0
c         do 23 k=1,nparmj
c  23       work(j)=work(j)+hessi(i,k)*hess_sav(k,j)
c  25   write(6,*) 'identity =', (work(j),j=1,nparmj)

c test solution
      do 30 i=1,nparmj
        grad_cal(i)=0
        do 30 j=1,nparmj
   30     grad_cal(i)=grad_cal(i)+hessi(i,j)*grad(j)
      write(6,'(''-x='',9g12.4)') (grad_cal(i),i=1,nparmj)
   ...
   49   read(1,*) (iwjasc(iparm,ict),iparm=1,nparmc(ict))
      ...
      do 50 ict=1,nctype
        do 50 i=1,nparma(ict)
          iparm=iparm+1
   50     a(iwjasa(i,ict),ict)=a(iwjasa(i,ict),ict)-grad_cal(indexi(iparm))
      do 60 i=1,nparmb(1)
        iparm=iparm+1
   60   b(iwjasb(i,1),1)=b(iwjasb(i,1),1)-grad_cal(indexi(iparm))
      do 70 ict=1,nctype
        do 70 i=1,nparmc(ict)
          iparm=iparm+1
   70     c(iwjasc(i,ict),ict,1)=c(iwjasc(i,ict),ict,1)-grad_cal(indexi(iparm))
      call cuspexact4(1)
...
c     parameter(NEQSX=2*(MORDJ-1),MTERMS=55)
      ...
      common /optim/ iwjasc(83,MCTYPE),nparmc(MCTYPE)
