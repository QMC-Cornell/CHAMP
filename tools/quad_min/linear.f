     ...
     &overlap_sav(MDIM,MDIM),coef(MDIM,MDIM)
      ...
      dimension work(MWORK),eigv(MDIM),eigvi(MDIM)
...
c invert overlap matrix
      call matinv(overlap,MDIM,nparmj)

      do 5 i=1,nparmj
        do 4 j=1,nparmj
          scratch(i,j)=0
          do 4 k=1,nparmj
    4       scratch(i,j)=scratch(i,j)+overlap(i,k)*overlap_sav(k,j)
    5   if((i.eq.j .and. abs(scratch(i,j)-1).gt.eps) .or.
     &     (i.ne.j .and. abs(scratch(i,j)).gt.eps)) stop 'inverse not right'

      if(idebug.gt.0) then
        do 6 i=1,nparmj
    6     write(6,'(''o      '',83e16.8)') (overlap_sav(i,j),j=1,nparmj)
        write(6,*)

        do 7 i=1,nparmj
    7     write(6,'(''o^-1*o '',83f6.2)') (scratch(i,j),j=1,nparmj)
        write(6,*)
...
c compute o^-1*H
      call prod(overlap,hmat,product,MDIM,nparmj)

      if(idebug.gt.0) then
        do 9 i=1,nparmj
    9    write(6,'(''o^-1*h '',20e10.2)') (product(i,j),j=1,nparmj)
        write(6,*)
      endif

c First check that MWORK is large enough
      call dgeev('N','V',nparmj,product,MDIM,eigv,eigvi,scratch, MDIM,
     &            coef, MDIM, work, -1,info) 
      write(6,'(''optimal MWORK='',f8.1)') work(1)
      if(work(1).gt.MWORK) stop 'work(1).gt.MWORK'
      call dgeev('N','V',nparmj,product,MDIM,eigv,eigvi,scratch, MDIM,
     &            coef, MDIM, work, MWORK,info) 
      if(info.ne.0) write(6,*) 'Warning dgeev: info =', info

      do 15 i=1,nparmj
   15   write(6,'(''eigenvalue '',f15.6,'' + i * '',f15.6)')
     &  eigv(i),eigvi(i)
      write(6,*)
...
c     nparm=0
      ...
      do 50 ict=1,nctype
        do 50 i=1,nparma(ict)
          iparm=iparm+1
   50     a(iwjasa(i,ict),ict)=a(iwjasa(i,ict),ict)+coef(iparm+1,i0)*dnorm
      do 60 i=1,nparmb(1)
        iparm=iparm+1
   60   b(iwjasb(i,1),1)=b(iwjasb(i,1),1)+coef(iparm+1,i0)*dnorm
      do 70 ict=1,nctype
        do 70 i=1,nparmc(ict)
          iparm=iparm+1
   70     c(iwjasc(i,ict),ict,1)=c(iwjasc(i,ict),ict,1)+coef(iparm+1,i0)*dnorm
      call cuspexact4(1)
