...
c Machine constants for RISK systems, at least the SGI R4000
c I commented out the next line and added the next 3 because even
c after using cpp, it would not compile.
c# include "/usr/include/float.h"
      parameter(DBL_MAX=1.7976931348623158d+308,
     & DBL_MIN=2.2250738585072014d-308,
     & DBL_EPSILON=2.2204460492503131d-16)

      double precision eps,dmn,dmx
      parameter (eps =  DBL_EPSILON )
      parameter (dmn = DBL_MIN )
      parameter (dmx = DBL_MAX )
      data dmach /eps, dmn, dmx/
c
