...
c     Written by C. Myers, 6/88, updated by C.M., J. Sethna and C. Umrigar
c     on 7/7/88, to remove artificial singularity in the s2 term associated
c     with collinearity of the three particles
c     Uses Fock expansion as described in:
c     Fock's Expansion, Kato's Cusp Conditions and the Exponential Ansatz,
c     C.R. Myers, C.J. Umrigar, J.P. Sethna and J.D. Morgan, PRA, 44, 5537 (1991).

c     GM calculates the helium ground state (singlet S) wave function
c     derived by Gottschalk and Maslen, J. Phys. A. 20, 2781 (1987);
c     parameters for this function are calculated in ~wfs/wfpars.f,
c     which should be called in the outermost driving program since it
c     need only be called once;
c     this function actually calculates not only the result in J. Phys. A.,
c     but also variants of that function generated by us,
c     distinguished by fflag (in common/fflags/) currently fflag = 6: GM,
c     fflag = 7: exp(GM - sub)

c     **Warning** The wavefucntion is evaluated correctly when the 2
c     electrons are collinear but not when u=0.
...
c     various configuration-dependent variables:
c     r = hyperradius (or its sqrt, depending on notation)
c     Y00, Y20 and Y21 are solid harmonics (IC equivalents of HH)
...
c-----------------------------------------------------------------------
      ...
      function xlob(xeval)
c Written by Chris Myers and Cyrus Umrigar
c Uses Fock expansion as described in:
c Fock's Expansion, Kato's Cusp Conditions and the Exponential Ansatz,
c C.R. Myers, C.J. Umrigar, J.P. Sethna and J.D. Morgan, PRA, 44, 5537 (1991).
...
c     implicit none
	 ...
	 goto 500
      ...
      eps2 = pib2 - xpii
      eps22 = eps2*eps2
c     ysing7 = xlobpi + eps2*(dlog(dabs(eps2)) - one
c    &   + eps2*eps2*(-(one/18.d0) + eps2*eps2*(-(one/900.d0)
c    &   + eps2*eps2*(-(one/19845.d0)))))
...
c phi21 originate terms in the real variables that contribute to phi31.
c The coeffs fck(10,1)-fck(15,1) are defined to cancel this contribution
c from phi31.
c If wf is a Pade', for small hyper-rad wf=P(1)(1-P(2)+..) and the 2nd
c order contribution from P(1)P(2) etc. has also to be subtracted out.

c WARNING*** we think that ijas2 is wrong in the phi31-like terms
c (scalel instead of two*scalel)
...
c Scaled variables in phi10 originate a 2nd order that contributes to phi20.
c The coeffs c(4,7,9) need to be redifined.
