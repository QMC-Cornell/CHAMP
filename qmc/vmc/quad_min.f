!     subroutine quad_min(energy,add_diag,force,force_err,npts,eig_min,eig_max)
!     subroutine quad_min(energy_sav,energy_err_sav,energy,energy_err,force,force_err,ene_var,add_diag,npts,eig_min,eig_max,p_var)
!     subroutine quad_min(energy_sav,energy_err_sav,ene_var,npts)
...
! In some toy tests a modification of this worked even better.
! Warning: at present we are fitting to the energy, even when we are optimizing a linear
! combination of energy and variance.  This results in add_diag gettting large if we
! start from a energy-minimized psi and then optimize the variance.  So, I should fit
! to the right linear combination.
...
!     dimension energy(npts),energy_err(npts),force(npts),force_err(npts),ene_var(npts),
!    &add_diag(npts),add_diag_log(npts),a(MFUNC,MFUNC),b(MFUNC)
...
!tmp    if(add_diag(k).lt.1.d99) then
!tmp      tmp=dlog10(add_diag(k)+max(eig_min,0.d0))
!tmp      add_diag_log(k)=tmp/(1+tmp/dlog10_eig_max)
!tmp     else
!tmp      add_diag_log(k)=dlog10(eig_max)
!tmp    endif
!tmp5 continue
...
!       energy_min=min(energy_min,energy(k))
...
! roughly determine if difference between energies is statistically significant
!     if((energy_max-energy_min)**2.gt.energy_err(1)**2+energy_err(2)**2+energy_err(3)**2) then
!       ienergy_diff_signif=1
!      else
!       ienergy_diff_signif=0
!     endif
...
! 1) if p_var<0.1 and force_err is small enough that force is significant, then make quadratic approximation
!    but limit it to changing add_diag(1) by no more than a factor of 100 in either direction
...
! 2) if p_var*(energy(2)+3*energy_err(2)-force_err(2))+q_var*energy_sigma(2)**2 <
!       p_var*(energy(1)+3*energy_err(1)             )+q_var*energy_sigma(1)**2 then use add_diag(2)
! 3) if p_var*(energy(1)+3*energy_err(1)             )+q_var*energy_sigma(1)**2 <
!       p_var*(energy(3)+3*energy_err(3)+force_err(3))+q_var*energy_sigma(3)**2 then use add_diag(1)
!    By having -force_err(2) in 2 and +force_err(3) in 3 we are favoring lower add_diag values when there
!    is no statistically significant difference.
! 3) otherwise use add_diag(3)
...
! The reason for 4) and 5) is mostly to prevent add_diag_min from getting large when optimizing
! scalek with variance minimization.
! We do not have the error bar on the variance so use the first part of foll.
! if statement only if we are doing mostly energy minimization
...
!c     elseif(energy(2).lt.energy_sav+sqrt(energy_err_sav**2+energy_err(2)**2)) then
!c      add_diag_log_min=add_diag_log(2)
!c     elseif(energy(1).lt.energy_sav+sqrt(energy_err_sav**2+energy_err(1)**2)) then
!c      add_diag_log_min=add_diag_log(1)
!      elseif(energy(2).lt.energy(1)+force_err(2) .and. ene_var_max-ene_var_min .lt. 1.d-3*abs(ene_var_max)) then
!       iwadd_diag=2
!       add_diag_log_min=add_diag_log(2)
!       if(energy(2).lt.energy(1) .and. add_diag_log_min.ge.0.d0) add_diag_log_min=add_diag_log_min-1.d0
!      elseif(energy(1).lt.energy(3)+force_err(3) .and. ene_var_max-ene_var_min .lt. 1.d-2*abs(ene_var_max) .and. k_min.eq.3) then
!       iwadd_diag=1
...
!     if(energy(2).lt.energy(1) .and. add_diag_log_min.ge.0.d0) then
!       add_diag_log_min=add_diag_log_min-1.d0
!       write(6,'(''aaaa'',9f12.5)') energy(2),energy(1),add_diag_log_min
!     endif
      write(6,'(''add_diag_min before bounding is'',1p,d12.4,0p)') 10**add_diag_log_min
!     add_diag_log_min=min(max(add_diag_log_min,-8.d0),6.d0)
      add_diag_log_min=max(add_diag_log_min,-12.d0)
      add_diag_min=10**add_diag_log_min
!tmp  add_diag_min=10**(add_diag_log_min/(1-add_diag_log_min/dlog10_eig_max))-max(eig_min,0.d0)
