...
! Warning: The replacemnt of the foll 2 lines by the above lines has not
! yet been tested
!         areao=(gammai(d3b2,zrtop,zetop)-gammai(d3b2,zrbot,zebot))
!    &    /(dexp(voldr*rmino(i))*(dsqrt(rmino(i)*(-voldr)**3)))
...
!       term=dmin1(voldp*rtry*sintht,one)
...
!       arean=(gammai(d3b2,zrtop,zetop)-gammai(d3b2,zrbot,zebot))
!    &  /(dexp(vnewr*rminn(i))*(dsqrt(rminn(i)*(-vnewr)**3)))

        endif
!       write(6,'(''fs'',2f7.1,2f7.2)') voldr,vnewr,voldp,vnewp
...
!       term=dmin1(vnewp*rmino(i)*sintht,one)
!       term=dmin1(vnewp*raver*sintht,one)
        term=vnewp*raver*sintht
! Determine cos(phi)
        cosphi=zero
        rnorm=zero
        do 160 k=1,ndim
! 160   cosphi=cosphi+(zaxis(k)-rvminn(k,i)/rminn(i))*xaxis(k)
...
!     write(6,'(19d12.4)') rmino(i),rtry,psi2o, psi2n,fxop,fxnp,areao
!    &,arean,p,enew(1)-eold
...
! Warning: have to set igeometrical
