...
* begin:
      ...
      if(lenstr.gt.0) then
         ...
         write(6,'(a,i10)')
     . ' error_CNVDBL> string length =',lenstr
         stop ' error_CNVDBL> string length < 0'

      else
         write(6,'(a)')
     . ' warning_CNVDBL> zero length string. converted to 0.d0'
         value = 0.d0
      end if
 ...
 999  continue
      write(6,'(3a)')
     .' warning_CNVDBL> conversion error for string "',
     . String(1:lenstr),'" converted to 1.d+99'
...
* -------------------------------------------------------------------- *
      ...
      if(lenstr.ne.0) then
         ...
         write(6,'(a)')
     . ' warning_CNVINT> zero length string. converted to 0'
         value = 0
      end if
...
* begin   :
      ...
      if(lenstr.eq.0) then
          write(6,'(a)')
     .'   error_sizstr> zero length string'
...
* search not blank from beginning.
      ...
      if(max) then
          write(6,'(a)')
     .  ' error_sizstr> strange  string'
          stop ' error_sizstr> strange  string'
      end if

      if(last.lt.first) then
          write(6,'(a,i5)')
     .  ' error_sizstr> first non-blank position=',first,
     .  ' error_sizstr> last  non-blank position=',last
