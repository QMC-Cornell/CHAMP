...
! Tau secondary in drift is one (first time around)
    ...
    5           v2old=v2old+voldw(k,i,iw,ifr)**2
              volda=sqrt(v2old)
              adrift=(half*(one+eps+voldr/volda)) + adrift0*hafzr2/(one+hafzr2)
...
! Tau primary -> tratio=one
          ...
          elseif(drift_type=='quadratic' .and. adrift > 1.d0) then
            vavvt=(dsqrt(one+two*adrift*v2old*tau*tratio)-one)/ (adrift*v2old +(adrift-1)*sqrt(2*v2old*adrift*tau))
          endif
...
!!        pgaus=1./(1+(znuc*pi/(zeta8**3*(zeta8-znuc)))*exp(-(xnuc-drift_tot)**2/(2*taudifr8))/((2*pi*taudifr8)**0.5*(2*pi*taudift8))) ; q8(zeta8)=1-p8(zeta8)
!         pgaus=1./(1+(znucc*pi/(zeta**3*(zeta-znucc)))*exp(-(max(rtry,0.d0))**2/(2*tau))/((2*pi*tau)**0.5d0*(2*pi*tau)))
!         qgaus=1-pgaus
!         write(6,'(''znucc,zeta,rmino,rtry,tau,pi='',9f10.6)') znucc,zeta,rmino,rtry,tau,pi
!         write(6,'(''pgaus_sav,pgaus='',9f10.6)') pgaus_sav,pgaus
!-------------------
...
! Use more accurate formula for the drift
          ...
          elseif(drift_type=='quadratic' .and. adrift > 1.d0) then
            vavvt=(dsqrt(one+two*adrift*v2new*tau*tratio)-one)/ (adrift*v2new +(adrift-1)*sqrt(2*adrift*v2new*tau))
          endif
