...
c     rteval_proc=dsqrt(eval_proc)
      ...
      write(6,'(''passes,eval,pass_proc,eval_proc,eval_proc_eff,
     &evalf_proc_eff,evalg_proc_eff'',19f9.0)')
...
c For the next 15 calls we switched from using mpi_reduce to mpi_allreduce
     ...
     &,mpi_sum,MPI_COMM_WORLD,ierr)
      call mpi_allreduce(wcum1,w1collect,1,mpi_double_precision
     &,mpi_sum,MPI_COMM_WORLD,ierr)
      call mpi_allreduce(wcm21,w21collect,1,mpi_double_precision
     &,mpi_sum,MPI_COMM_WORLD,ierr)
      ...
      call mpi_allreduce(efcum1,ef1collect,1,mpi_double_precision
     &,mpi_sum,MPI_COMM_WORLD,ierr)
      call mpi_allreduce(efcm21,ef21collect,1,mpi_double_precision
     &,mpi_sum,MPI_COMM_WORLD,ierr)
      call mpi_allreduce(wfcum1,wf1collect,1,mpi_double_precision
     &,mpi_sum,MPI_COMM_WORLD,ierr)
      call mpi_allreduce(wfcm21,wf21collect,1,mpi_double_precision
     &,mpi_sum,MPI_COMM_WORLD,ierr)
...
c    &eval,passes,nconf_global,nstep,iblk,nblkeq,nproc,tau,taucum(1)/wgcum(1)
     ...
     &  wfave,wferr,wferr*rtpass_proc1,wferr1*rtpass_proc1,
     &  (wferr/wferr1)**2
        do 20 ifr=1,nforce
     ...
     &    wgave,wgerr,wgerr*rtpass_proc1,wgerr1*rtpass_proc1,
     &    (wgerr/wgerr1)**2
  20    continue
        write(6,'(''total energy (   0) ='',t22,f14.7,'' +-'',f11.7,
     &  2f9.5,f8.2)') eave,eerr,eerr*rteval_proc_eff1,
     &  eerr1*rteval_proc_eff1,(eerr/eerr1)**2
        write(6,'(''total energy (   1) ='',t22,f14.7,'' +-'',f11.7,
     &  2f9.5,f8.2)') efave,eferr,eferr*rtevalf_proc_eff1,
     &  eferr1*rtevalf_proc_eff1,(eferr/eferr1)**2
      endif
      do 30 ifr=1,nforce
...
c save energy, energy_sigma and energy_err for optimization
     ...
     &    f11.7,2f9.5,f8.2)') nfprod,egave,egerr,egerr*rtevalg_proc_eff1,
     &    egerr1*rtevalg_proc_eff1,(egerr/egerr1)**2
         else
          write(6,'(''total energy ('',i4,'')'',i1,''='',t22,f14.7,'' +-'',
     &    f11.7,2f9.5,f8.2)') nfprod,ifr,egave,egerr,egerr*rtevalg_proc_eff1,
     &    egerr1*rtevalg_proc_eff1,(egerr/egerr1)**2
        endif
  30  continue
...
c         peerr=peerr+tmerr     ! is this correct?
     ...
     &  ,f11.7,f9.5)') peave,peerr,peerr*rtevalg_proc_eff1
        write(6,'(''interaction energy ='',t22,f14.7,'' +-''
     &  ,f11.7,f9.5)') peiave,peierr,peierr*rtevalg_proc_eff1
        write(6,'(''jf kinetic energy ='',t22,f14.7,'' +-''
     &  ,f11.7,f9.5)') tjfave,tjferr,tjferr*rtevalg_proc_eff1
        write(6,'(''pb kinetic energy ='',t22,f14.7,'' +-''
     &  ,f11.7,f9.5)') tpbave,tpberr,tpberr*rtevalg_proc_eff1
...
c save energy difference and error in energy difference for optimization
     ...
     &  ,'' +-'',f11.7,f9.5)') ifr,fgave,fgerr,fgerr*rtevalg_proc_eff1
  50  continue
