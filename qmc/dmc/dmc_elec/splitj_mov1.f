...
c     write(6,'(''nwalk at beg of splitjoin'',9i5)') nwalk
      ...
      do 10 iw=1,nwalk
        ...
        if(wt(iw).lt.half) then
          if(wt(iw).eq.zero) then
            ...
            iunder=iunder+1
            iwundr(iunder)=iw
c           write(6,'(''Eliminating walker'',i5)') iw
           else
            if(ipair.eq.0) then
              ipair=1
              iw2=iw
...
c             write(6,'(''Joined walkers'',9i5)') iwundr(iunder),iw2,iw
            endif
          endif
        endif
   10 continue
...
c Warning: MWALK can still be exceeded on very rare occasions if a walker gets split more than once.
      ...
      if(iunder.gt.0) then
c       write(6,'(''Number of walkers joined,nwalk,nwalk_after_join'',9i5)') iunder,nwalk,nwalk_after_join
c       write(6,'(''Holes are at walkers'',999i5)') (iwundr(i),i=1,iunder)
      endif
c     write(6,'(''wt_split1='',f8.2)') wt_split
...
c     write(6,'(''wt_split2='',f8.2)') wt_split
...
c         if(iage(iw2).ge.1) write(6,'(''iage,wt='',i5,f8.2)') iage(iw2),wt(iw2)
c         write(6,'(''iage,iw2,wt,e,v2='',2i5,9f9.2)') iage(iw2),iw2,wt(iw2),eoldw(iw2,1),v2
        endif
   20 continue
...
c the remaining walkers so that they are they occupy the first positions.
      do 30 j=iunder,1,-1
        iw2=iwundr(j)
        iw=nwalk2
c       write(6,'(''Moving walker'',i5,'' to walker'',i5)') iw,iw2
        nwalk2=nwalk2-1
        wt(iw2)=wt(iw)
        iage(iw2)=iage(iw)
        if(ibasis.eq.3) then            !complex basis set
...
c     write(6,'(''nwalk at end of splitjoin'',9i5)') nwalk
