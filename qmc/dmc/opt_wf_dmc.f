...
! Do dmc computing gradient and hessian but no correlated sampling
          igradhess=1
          nforce=1
          nwftype=1
          iadd_diag_loop1=0
...
! Warning: I need to fix the foll. to reset mc_configs in dmc
...
! Warning: I need to fix the foll. to reset mc_configs
! just in case mc config is in crazy place, reset mc_configs by calling sites
!               isite=1
!               call mc_configs_read
                goto 408
...
! end of 1st if(iadd_diag_opt.eq.1) then
...
! end of 2nd if(iadd_diag_opt.eq.1) then
