...
!JT     &,iwctype(MCENT),nctype,ncent
     ...
     &psijn(MFORCE),enew(MFORCE),pen(MFORCE),pein(MFORCE),d2n(MFORCE),div_vn(MELEC)
      dimension xaxis(3),zaxis(3),xbac(3),ajacnew(MFORCE)
...
c Convert back to original coordinate system
   ...
   60       continue
            ...
            if(ipr.ge.1) then
              write(6,'(''vav,volda,driftr,rtry'',9f9.5)')
     &        vav,volda,driftr,rtry,xprime,zprime
...
c Sample gaussian with prob pgaus, exponential with prob. qgaus
  ...
  100       fnormo=fnormo*(pgaus*gaus_norm*exp(-half*dfus2a/tau_hom)
...
c           write(6,'(''n pgaus,qgaus='',4d9.2)') pgaus,qgaus,qgaus2
     ...
     &      +qgaus*(zeta**3/pi)*dexp(-two*zeta*dfusb))
            ...
            if(ipr.ge.1) then
              write(6,'(''xoldw'',9f10.6)')(xoldw(k,i,iw,ifr),k=1,ndim),
