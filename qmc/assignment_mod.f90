...
!-------------------------------------------------------------------------------
    subroutine assign_elecs(x,method)
       ...
       use atom_mod, only: znuc, iwctype, cent, ncent
       use fragments_mod, only: iwfragelec, iwfragnucl
       use const_mod, only: ipr
       use all_tools_mod, only: die
       implicit none
       real(dp) :: x(3,*)
       integer :: method
       ...
       enddo
       if(znuc_tot.NE.nelec) call die ('assign_elecs', 'Fragments not yet implemented for ionic systems.') 
       ...
       case (1)
          ...
          call heap(iwfragelec)
       case(2)
          ! Munkres' (Hungarian) algorithm
          call munkres(iwfragelec)
       case default
       ...
       if (ipr.GE.1) then
          ...
          sumSol = 0
          do i = 1, n
             sumSol = sumSol + CC(iwfragelec(i),i)
          enddo
          ...
          sum = 0
          do i = 1, n
             sum = sum + CC(iwfragelec(i),i)
             write(6,'(i4,x,i4,x,f6.3,x,f9.3)') i, iwfragelec(i), CC(iwfragelec(i),i), sum
          enddo
       endif

       do i=1,nelec
          !After heap/munkres is called, iwfragelec(i) is the 'job' index j 
          !assigned to electron i.
           
          !The nucleus corresponding to job j is nuclj(j), so 
          !nuclj(iwfragelec(i)) is the nucleus assigned to electron i.
           
          !The fragment corresponding to nucleus ic is iwfragnucl(ic), so 
          !iwfragnucl(nuclj(iwfragelec(i))) gives the fragment assigned to 
          !electron i.
          iwfragelec(i)=iwfragnucl(nuclj(iwfragelec(i)))
       enddo
