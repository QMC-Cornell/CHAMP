...
! loop over menu lines
  ...
  case ('elec_nb_closest_to_atom')
   call get_next_value_list ('elec_nb_closest_to_atom_input', &
        elec_nb_closest_to_atom_input, elec_nb_closest_to_atom_input_nb)
   call object_provide ('ncent')
   if (elec_nb_closest_to_atom_input_nb /= ncent) then
    write(6,'(3a,i3,a,i3)') 'Fatal error in routine ',&
        trim(lhere),': ',elec_nb_closest_to_atom_input_nb,&
        ' values read in list elec_nb_closest_to_atom while the expected number was the number of atom centers = ',ncent
   call die (lhere)
   endif
   call object_modified ('elec_nb_closest_to_atom_input')

  case ('elec_up_nb_closest_to_atom')
   call get_next_value_list ('elec_up_nb_closest_to_atom_input', &
        elec_up_nb_closest_to_atom_input, elec_up_nb_closest_to_atom_input_nb)
   call object_provide ('ncent')
   if (elec_up_nb_closest_to_atom_input_nb /= ncent) then
    write(6,'(3a,i3,a,i3)') 'Fatal error in routine ',&
        trim(lhere),': ',elec_up_nb_closest_to_atom_input_nb,&
        ' values read in list elec_up_nb_closest_to_atom while the expected number was the number of atom centers = ',ncent
   call die (lhere)
   endif
   call object_modified ('elec_up_nb_closest_to_atom_input')

  case ('elec_dn_nb_closest_to_atom')
   call get_next_value_list ('elec_dn_nb_closest_to_atom_input', &
        elec_dn_nb_closest_to_atom_input, elec_dn_nb_closest_to_atom_input_nb)
   call object_provide ('ncent')
   if (elec_dn_nb_closest_to_atom_input_nb /= ncent) then
    write(6,'(3a,i3,a,i3)') 'Fatal error in routine ',&
        trim(lhere),': ',elec_dn_nb_closest_to_atom_input_nb,&
        ' values read in list elec_dn_nb_closest_to_atom while the expected number was the number of atom centers = ',ncent
   call die (lhere)
   endif
...
! if walker file does not exist
     ...
     if (idmc < 0) then
       ...
       call die (lhere, 'Walker file does not exist >'+&
        trim(file_mc_configs_in)+'<. First run in vmc mode to generate it.')
     endif ! if idmc <0
  ...
  do walk_i = 1, nconf
    do elec_i = 1, nelec
      do elec_j = elec_i + 1, nelec
         ...
         if (dist < 1.d-24) then
            dist=sqrt(dist)
            write(6,'(a,i5,a,i3,a,i3,a)') 'Error: in walker # ',walk_i,&
                 ', electrons ',elec_i,' and ',elec_j, ' are too close.'
...
! loop over lines in file
     call alloc ('coord_elec_wlk_read', coord_elec_wlk_read, ndim, nelec, &
        nconf_total)
     do
        read(file_unit,fmt=*,iostat=iostat) ((coord_elec_read &
             (dim_i, elec_i), dim_i=1,ndim), elec_i=1,nelec)
        if (iostat /= 0) exit
...
!     call release ('coord_elec_read', coord_elec_read)
     ...
     write(6,'(i5,3a)') nconf_read,' walkers read from file >',&
        trim(file_mc_configs_in),'<'
...
! Check uniqueness of walkers
  ...
  do walk_i = 1, nconf_read
    ...
    do walk_j = walk_i +1, nconf_read
     if (arrays_equal (coord_elec_wlk_read (:,:, walk_i), &
        coord_elec_wlk_read (:,:, walk_j))) then
      ...
      do elec_j = elec_i+1, nelec
        ...
        do dim_i = 1, ndim
          dist_ee_wlk_read = dist_ee_wlk_read &
               + (coord_elec_wlk_read (dim_i, elec_i, walk_i) - &
               coord_elec_wlk_read (dim_i, elec_j, walk_i))**2
        enddo ! dim_i
...
!        dist_ee_wlk_read (elec_j, elec_i, walk_i) = dist_ee_wlk_read (elec_i, elec_j, walk_i)
        if (dist_ee_wlk_read < 1.d-6) then
           ...
           write(6,'(a,i5,a,i3,a,i3,a)') 'Error: in walker # ',walk_i,&
                ', electrons ',elec_i,' and ',elec_j, ' are too close.'
           write(6,'(a,es15.8)') 'They are at a distance of ',dist_ee_wlk_read,&
                ' < 1.d-6.'
...
!       dist_en_wlk_read (elec_i, cent_i, walk_i) = dsqrt (dist_en_wlk_read (elec_i, cent_i, walk_i))
       if (dist_en_wlk_read < 1.d-6) then
           write(6,*)
           write(6,'(a,i5,a,i3,a,i3)') 'Warning: in walker # ',walk_i,&
                ', electron #',elec_i,' is very close to nucleus #',cent_i
           write(6,'(a,es15.8)') 'They are at a distance of ',dist_en_wlk_read,&
                ' < 1.d-6.'
       endif
      enddo ! elec_i
   enddo ! cent_i
...
! Check number of electrons closest to each atoms
  ...
  call alloc ('elec_nb_closest_to_atom_wlk_read', &
       elec_nb_closest_to_atom_wlk_read, ncent, nconf_read)
  call alloc ('elec_up_nb_closest_to_atom_wlk_read', &
        elec_up_nb_closest_to_atom_wlk_read, ncent, nconf_read)
  call alloc ('elec_dn_nb_closest_to_atom_wlk_read', &
        elec_dn_nb_closest_to_atom_wlk_read, ncent, nconf_read)
  ...
  do walk_i = 1, nconf_read
   do elec_i = 1, nelec
     ...
     enddo ! cent_i
     if (elec_i <= nup) then
      elec_up_nb_closest_to_atom_wlk_read (cent_closest_i, walk_i) =&
          elec_up_nb_closest_to_atom_wlk_read (cent_closest_i, walk_i) + 1
     else
      elec_dn_nb_closest_to_atom_wlk_read (cent_closest_i, walk_i) =&
          elec_dn_nb_closest_to_atom_wlk_read (cent_closest_i, walk_i) + 1
     endif
      elec_nb_closest_to_atom_wlk_read (cent_closest_i, walk_i) = &
         elec_nb_closest_to_atom_wlk_read (cent_closest_i, walk_i) + 1
   enddo ! elec_i
  enddo ! walk_i
  elec_nb_closest_to_atom_read_nb = 1
  elec_up_nb_closest_to_atom_read_nb = 1
  elec_dn_nb_closest_to_atom_read_nb = 1
  call alloc ('elec_nb_closest_to_atom_read', elec_nb_closest_to_atom_read,&
         ncent, elec_nb_closest_to_atom_read_nb)
  call alloc ('elec_up_nb_closest_to_atom_read', &
        elec_up_nb_closest_to_atom_read, ncent, &
        elec_up_nb_closest_to_atom_read_nb)
  call alloc ('elec_dn_nb_closest_to_atom_read',&
          elec_dn_nb_closest_to_atom_read, ncent,&
         elec_dn_nb_closest_to_atom_read_nb)
  call alloc ('elec_nb_closest_to_atom_wlk_read_nb', &
        elec_nb_closest_to_atom_wlk_read_nb, elec_nb_closest_to_atom_read_nb)
  call alloc ('elec_up_nb_closest_to_atom_wlk_read_nb',&
         elec_up_nb_closest_to_atom_wlk_read_nb, elec_up_nb_closest_to_atom_read_nb)
  call alloc ('elec_dn_nb_closest_to_atom_wlk_read_nb',&
         elec_dn_nb_closest_to_atom_wlk_read_nb, elec_dn_nb_closest_to_atom_read_nb)
  elec_nb_closest_to_atom_read (:, elec_nb_closest_to_atom_read_nb) =&
         elec_nb_closest_to_atom_wlk_read (:, 1)
  elec_up_nb_closest_to_atom_read (:, elec_up_nb_closest_to_atom_read_nb) =&
         elec_up_nb_closest_to_atom_wlk_read (:, 1)
  elec_dn_nb_closest_to_atom_read (:, elec_dn_nb_closest_to_atom_read_nb) =&
         elec_dn_nb_closest_to_atom_wlk_read (:, 1)
  ...
  do walk_i = 2, nconf_read
    ...
    do elec_nb_closest_to_atom_read_i = 1, elec_nb_closest_to_atom_read_nb
      if (arrays_equal (elec_nb_closest_to_atom_wlk_read (:, walk_i), &
        elec_nb_closest_to_atom_read (:, elec_nb_closest_to_atom_read_i))) then
         elec_nb_closest_to_atom_wlk_read_nb (elec_nb_closest_to_atom_read_i)&
         = elec_nb_closest_to_atom_wlk_read_nb (elec_nb_closest_to_atom_read_i) + 1
        found = .true.
        exit
      endif
    enddo
   if (.not. found) then
     ...
     call alloc ('elec_nb_closest_to_atom_wlk_read_nb',&
         elec_nb_closest_to_atom_wlk_read_nb, elec_nb_closest_to_atom_read_nb)
     elec_nb_closest_to_atom_read (:, elec_nb_closest_to_atom_read_nb) =&
         elec_nb_closest_to_atom_wlk_read (:, walk_i)
     elec_nb_closest_to_atom_wlk_read_nb (elec_nb_closest_to_atom_read_nb) = 1
   endif
  enddo ! walk_i
  ...
  do walk_i = 2, nconf_read
    ...
    do elec_up_nb_closest_to_atom_read_i = 1, elec_up_nb_closest_to_atom_read_nb
      if (arrays_equal (elec_up_nb_closest_to_atom_wlk_read (:, walk_i), &
        elec_up_nb_closest_to_atom_read (:, elec_up_nb_closest_to_atom_read_i))) then
         elec_up_nb_closest_to_atom_wlk_read_nb &
        (elec_up_nb_closest_to_atom_read_i) = &
        elec_up_nb_closest_to_atom_wlk_read_nb (elec_up_nb_closest_to_atom_read_i) + 1
        found = .true.
        exit
      endif
    enddo
   if (.not. found) then
     ...
     call alloc ('elec_up_nb_closest_to_atom_read', &
        elec_up_nb_closest_to_atom_read, ncent, elec_up_nb_closest_to_atom_read_nb)
     call alloc ('elec_up_nb_closest_to_atom_wlk_read_nb',&
         elec_up_nb_closest_to_atom_wlk_read_nb, elec_up_nb_closest_to_atom_read_nb)
     elec_up_nb_closest_to_atom_read (:, &
        elec_up_nb_closest_to_atom_read_nb) = elec_up_nb_closest_to_atom_wlk_read (:, walk_i)
     elec_up_nb_closest_to_atom_wlk_read_nb (elec_up_nb_closest_to_atom_read_nb) = 1
   endif
  enddo ! walk_i
  ...
  do walk_i = 2, nconf_read
    ...
    do elec_dn_nb_closest_to_atom_read_i = 1, elec_dn_nb_closest_to_atom_read_nb
      if (arrays_equal (elec_dn_nb_closest_to_atom_wlk_read (:, walk_i),&
         elec_dn_nb_closest_to_atom_read (:, elec_dn_nb_closest_to_atom_read_i))) then
         elec_dn_nb_closest_to_atom_wlk_read_nb (elec_dn_nb_closest_to_atom_read_i) = &
        elec_dn_nb_closest_to_atom_wlk_read_nb (elec_dn_nb_closest_to_atom_read_i) + 1
        found = .true.
        exit
      endif
    enddo
   if (.not. found) then
     ...
     call alloc ('elec_dn_nb_closest_to_atom_read', &
        elec_dn_nb_closest_to_atom_read, ncent, elec_dn_nb_closest_to_atom_read_nb)
     call alloc ('elec_dn_nb_closest_to_atom_wlk_read_nb', &
        elec_dn_nb_closest_to_atom_wlk_read_nb, elec_dn_nb_closest_to_atom_read_nb)
...
!  enddo
  ...
  do elec_nb_closest_to_atom_read_i = 1, elec_nb_closest_to_atom_read_nb
     write(6,'(i5,a,100f7.0)') elec_nb_closest_to_atom_wlk_read_nb &
        (elec_nb_closest_to_atom_read_i),&
        ' walkers have the following numbers of electrons closest to each atom:',&
         elec_nb_closest_to_atom_read (:, elec_nb_closest_to_atom_read_i)
  enddo
  ...
  do elec_up_nb_closest_to_atom_read_i = 1, elec_up_nb_closest_to_atom_read_nb
     write(6,'(i5,a,100f7.0)') elec_up_nb_closest_to_atom_wlk_read_nb &
        (elec_up_nb_closest_to_atom_read_i), &
        ' walkers have the following numbers of spin-up electrons closest to each atom:', &
        elec_up_nb_closest_to_atom_read (:, elec_up_nb_closest_to_atom_read_i)
  enddo
  if (elec_up_nb_closest_to_atom_read_nb > 1) then
     write(6,'(a)') &
        'Warning: all walkers do not have the same numbers of spin-up electrons closest to each atom.'
  endif

  do elec_dn_nb_closest_to_atom_read_i = 1, elec_dn_nb_closest_to_atom_read_nb
     write(6,'(i5,a,100f7.0)') elec_dn_nb_closest_to_atom_wlk_read_nb (elec_dn_nb_closest_to_atom_read_i), &
   ' walkers have the following numbers of spin-down electrons closest to each atom:', &
        elec_dn_nb_closest_to_atom_read (:, elec_dn_nb_closest_to_atom_read_i)
  enddo
...
! Drop walkers with electron distributions on atoms different than requested in the input
  if (l_keep_only_walkers_with_elec_nb_closest_to_atom_input) then
   ...
   nconf_dropped = 0
   do walk_i = 1, nconf_read
    if (.not. arrays_equal (elec_nb_closest_to_atom_wlk_read (:, walk_i), &
        elec_nb_closest_to_atom_input (:))) then
      call remove_elt_in_array (coord_elec_wlk_read, walk_i - nconf_dropped)
      nconf_dropped = nconf_dropped + 1
!      write(6,'(2a,i5,a,100f7.0)') trim(lhere),': drop walker #', walk_i,',&
!         number of closest electrons =',elec_nb_closest_to_atom_wlk_read (:, walk_i)
    endif
   enddo
   nconf_read = nconf_read - nconf_dropped
   write (6, '(a, i5,a,100f7.0)') 'Warning:',nconf_dropped,&
        ' walkers are dropped that do not have the requested numbers of electrons closest to each atom:',&
         elec_nb_closest_to_atom_input (:)
  endif
...
! Save walkers (and duplicate walkers if necessary):
  ...
  if (nconf_total > nconf_read) then
   ...
   write(6,'(a,i5,a)') 'Warning: not enough walkers. ', nconf_missing,&
        ' remaining walkers duplicated.'
  endif
