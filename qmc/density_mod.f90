...
! For periodic densities in fourier space
  ...
  logical                           :: l_dens_fourier_real = .false.
  real(dp), allocatable             :: dens_fourier_real (:)
  real(dp), allocatable             :: dens_fourier_real_av (:)
  real(dp), allocatable             :: dens_fourier_real_av_err (:)
...
! loop over menu lines
  ...
  case ('help')
   ...
   write(6,'(a)') '  real    = [bool]   : compute real part only (default:false)'
   write(6,'(a)') '  file    = [string] : file in which density will be written'
   write(6,'(a)') 'end'

  case ('real')
   call get_next_value (l_dens_fourier_real)
...
! File
  ...
  if (l_dens_fourier_real) then
    write(6,'(a)') ' compute only real part of density Fourier components'
    call object_average_request ('dens_fourier_real_av')
    call object_error_request ('dens_fourier_real_av_err')
  else
...
! ==============================================================================
  subroutine dens_fourier_real_bld
! ------------------------------------------------------------------------------
! Description   : estimator of periodic density (real part of Fourier transform)
!
...
! header
  if (header_exe) then
   ...
   call object_create ('dens_fourier_real')
   call object_average_define ('dens_fourier_real', 'dens_fourier_real_av')
   call object_error_define ('dens_fourier_real_av', 'dens_fourier_real_av_err')
...
! allocations
  call object_alloc ('dens_fourier_real', dens_fourier_real, ngvec_big)
  call object_alloc ('dens_fourier_real_av', dens_fourier_real_av, ngvec_big)
  call object_alloc ('dens_fourier_real_av_err', dens_fourier_real_av_err, ngvec_big)

  do gvec_i = 1, ngvec_big

    dens_fourier_real(gvec_i) = 0.d0
    ...
    dens_fourier_real(gvec_i) = dens_fourier_real(gvec_i) + dcos(dotproduct)
  ...
  end subroutine dens_fourier_real_bld
...
! open file
  ...
  write(unit,'(a,i15)')     'number of electrons       =',nelec
...
! open file
  ...
  write(unit,'(a,i12)')     'number of electrons       =',nelec
  write(unit,'(a,i12)')     'number of walkers         =',nwalk
  write(unit,'(a,i12)')     'number of steps per block =',nstep_total
  write(unit,'(a,i12)')     'number of blocks          =',block_iterations_nb
...
! open file
  ...
  write(unit,'(9a)')        'Density generated by CHAMP using estimator "',trim(dens_3d_estimator),'" and mode "',trim(mode)
  write(unit,'(a,i12)')     'number of electrons       =',nelec
  write(unit,'(a,i12)')     'number of walkers         =',nwalk
  write(unit,'(a,i12)')     'number of steps per block =',nstep_total
  write(unit,'(a,i12)')     'number of blocks          =',block_iterations_nb
...
! open file
  ...
  write(unit,'(9a)')        'Fourier density components generated by CHAMP using mode ',trim(mode)
  write(unit,'(a,i10)')     'number of electrons       =',nelec
  write(unit,'(a,i10)')     'number of walkers         =',nwalk
  write(unit,'(a,i10)')     'number of steps per block =',nstep_total
  write(unit,'(a,i10)')     'number of blocks          =',block_iterations_nb
  write(unit,'(a,i10)')     'number of primitive gvec  =',ngvec_big
