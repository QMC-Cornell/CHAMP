...
! open file
  ...
  write(unit,'(9a)')        '#D fourier density components generated by CHAMP using mode ',trim(mode)
  write(unit,'(a,i5)')      'number of electrons       =',nelec
  write(unit,'(a,i20)')     'number of walkers         =',nwalk
  write(unit,'(a,i20)')     'number of steps per block =',nstep_total
  write(unit,'(a,i20)')     'number of blocks          =',block_iterations_nb
  write(unit,'(a,i12)')     'number of primitive gvec  =',ngvec_big

  write(unit,*) ''
  if(l_dens_fourier_real) then
    write(unit,'(a)') '                        gvec                real part of density Fourier components       statistical error'
    do gvec_i = 1, ngvec_big
